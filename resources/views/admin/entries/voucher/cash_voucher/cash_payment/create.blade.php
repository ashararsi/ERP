        ...
        <div class="card">
            <div class="card-body">
                <div class="main-content-label mg-b-5">
                    <h3 style="float:left;">Create Cash Payment Voucher</h3>
                    <a href="{{ route('admin.entries.index') }}" style="float:right;"
                       class="btn btn-success pull-right">Back</a>
                </div>
                <!-- /.box-header -->
                <!-- form start -->
                {!! Form::open(['method' => 'POST', 'route' => ['admin.cpv-store'], 'id' => 'validation-form']) !!}
                <div class="box-body" style="margin-top:40px;">
                    {{--                    @include('accounts.entries.voucher.cash_voucher.cash_payment.fields',['companyId' => $companyId,'branchId' => $branchId])--}}
                    {!! Form::hidden('entry_type_id', old('entry_type_id', '3'),['id'=>'entry_type_id']) !!}
                    @include('accounts.entries.voucher.journal_voucher.fields',['companyId' => $companyId,'branchId' => $branchId,'vendor'=>$vendor,'vendorDropdown'=>$vendorDropdown])
                    <div class="row">
                    ...
                    {!! Form::close() !!}
                    {{--                    @include('accounts.entries.voucher.cash_voucher.cash_payment.intrument_entries_template')--}}
                                        ...
                                        <td>
                                            ...
                                            @if(Auth::user()->isAbleTo('accounts-edit-voucher'))
                                                <a class="btn btn-success"
                                                   href="{!! url('admin/cpv-edit/'.$item->id) !!}">Edit</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
