...
var FormControlsssss = function () {
    ...
    var baseFunction = function () {
        ...
        $('.base-data-ajax').select2({
            ...
            ajax: {
                url: route('admin.voucher.bank_search'),
                dataType: 'json',
                delay: 500,
    ...
    var Select2AjaxObj = function () {
        ...
        return {
            ...
            minimumInputLength: 2,
            ajax: {
                url: route('admin.voucher.crv_search'),
                dataType: 'json',
                delay: 500,
                data: function (params) {
                    return {
    ...
    var createEntryItem = function () {
        ...
        var global_counter = parseInt($('#utilitybill_entry_item-global_counter').val()) + 1;
        var entry_item = $('#utilitybill_entry_item-container').html().replace(/########/g, '').replace(/######/g, global_counter);
        $('#utilitybill_entry_item tr:last').before(entry_item);

        $('#utilitybill_entry_item-ledger_id-'+global_counter).select2(Select2AjaxObj());
        $('#utilitybill_entry_item-global_counter').val(global_counter)
    }

    var destroyEntryItem = function (itemId) {
        var r = confirm("Are you sure to delete Entry Item?");
        if (r == true) {
            $('#utilitybill_entry_item-ledger_id-'+itemId).select2(Select2AjaxObj());
            $('#utilitybill_entry_item-'+itemId).remove();
            CalculateTotal();
        }
    }
