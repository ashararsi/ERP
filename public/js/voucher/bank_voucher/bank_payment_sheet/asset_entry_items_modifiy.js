...
var FormControlssss1 = function () {
    ...
    var createEntryItem = function () {
        var global_counter = parseInt($('#asset_entry_item-global_counter').val()) + 1;
        var entry_item = $('#asset_entry_item-container').html().replace(/########/g, '').replace(/######/g, global_counter);
        $('#asset_entry_items tr:last').before(entry_item);
        // Apply Select2 on newly created item
        $('#asset_entry_item-ledger_id-'+global_counter).select2(Select2AjaxObj());
        $('#asset_entry_item-global_counter').val(global_counter)
    }

    var destroyEntryItem = function (itemId) {
        var r = confirm("Are you sure to delete Entry Item?");
        if (r == true) {
            $('#asset_entry_item-ledger_id-'+itemId).select2(Select2AjaxObj());
            $('#asset_entry_item-'+itemId).remove();
            CalculateTotal();
        }
    }
