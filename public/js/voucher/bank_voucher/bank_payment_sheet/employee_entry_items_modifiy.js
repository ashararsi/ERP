...
var FormControlsss = function () {
    ...
    var createEntryItem = function () {
        var global_counter = parseInt($('#employee_entry_item-global_counter').val()) + 1;
        var entry_item = $('#employee_entry_item-container').html().replace(/########/g, '').replace(/######/g, global_counter);
        $('#employee_entry_items tr:last').before(entry_item);
        // Apply Select2 on newly created item
        $('#employee_entry_item-ledger_id-'+global_counter).select2(Select2AjaxObj());
        $('#employee_entry_item-global_counter').val(global_counter)
    }
