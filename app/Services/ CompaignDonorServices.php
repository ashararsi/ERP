...
class compaignDonorServices
{
    public function index($request)
    {
        $perPage = $request->get('per_page', 20);
        $posts = CampaignDonors::orderBy('created_at', 'desc')->paginate($perPage);
    ...
    public function store($request)
    {
        ...
        return CampaignDonors::create($validated);

    }

    public function edit($id)
    {
        return $post = CampaignDonors::findOrFail($id);
    ...
    public function update($request, $id)
    {
        ...
        $transaction = CampaignDonors::find($id);
        if (!$transaction) {
            return response()->json([
    ...
    {
        $Transaction = CampaignDonors::findOrFail($id);
        if ($Transaction) {
            $Transaction->delete();
        }
    }
